...
from srcs.linear_regression.plotter_class import PlottingClass
...
class LinearRegressionModel:
    ...
    def load_data(
        self, data_path: str, x_feature: str, y_feature: str
    ) -> None:
        """
        ...
        self.__load_data()
        self.plotter = PlottingClass(
            x_feature=self.x,
            x_name=self.x_feature,
            y_feature=self.y,
            y_name=self.y_feature,
    ...
    def __standardize_features(self) -> None:
        ...
        try:
            ...
            raise ZeroDivisionError(
                "Standard deviation of the feature to predict is 0"
            ) from e

    def __initialize_progress_bar(
        self, message: str = "", total: int = 0
    ) -> None:
        """
        ...
        """
        self.pbar = tqdm.tqdm(
            total=total, desc=message, position=0, leave=True
        )

    def fit(self, visualize: bool = False) -> ndarray:
        """
    ...
    def __animate_data(self):
        ...
        self.__initialize_progress_bar(
            "Creating the animation...", self.animation_epoch
        )
    ...
    def __gradient_descent(self) -> None:
        ...
        for i in range(self.iter):
            ...
            if (
                len(self.costs) > 1
                and abs(self.costs[-1] - self.costs[-2]) < self.stop_threshold
            ):
    ...
    def __de_standardize_theta_history(self) -> None:
        ...
        de_standardized_theta0_history = (
            (np.array(self.theta0_history) * self.y_std)
            + self.y_mean
            - (
    ...
    @staticmethod
    def __cost_function(
        prediction: np.ndarray, y: np.ndarray, n: int
    ) -> float:
        """
    ...
    def __de_standardize_theta(self) -> None:
        ...
        de_standardized_theta0 = (
            (theta0_standardized * self.y_std)
            + self.y_mean
            - (theta1_standardized * self.x_mean * self.y_std / self.x_std)
        )
    ...
    def plot_data(self) -> None:
        ...
        self.plotter.plot_data(
            self.x, self.y, hypothesis, self.x_feature, self.y_feature
        )
...
def premade_data() -> None:
    ...
    model.load_data(
        data_path=data_path, x_feature=x_feature, y_feature=y_feature
    )
